#include "stdafx.h" // #include "pch.h" // 预编译头
...
bool Merge::addStream()
{
	...
	if (wantIndex.video_codecId != AV_CODEC_ID_NONE) {
		...
		AVRational time_base;
		time_base.num = 1;
		time_base.den = wantIndex.fps * 100;

		AVRational framerate;
		framerate.num = wantIndex.fps;
		framerate.den = 1;
			...
			while (true) {
				...
				if (first) {
					...
					if (left_size >= (*dst)->nb_samples) {
						...
						AVRational pts;
						pts.num = 1;
						pts.den = (*dst)->sample_rate;
						(*dst)->pts = av_rescale_q(next_audio_pts, pts, audio_en_ctx->time_base);
