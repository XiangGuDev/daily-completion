...
*/
const bool MPP09Reader::DEFAULT_WORKING_WEEK[] =
{
	false,
	true,
	true,
...
//MppBitFlagArrayPtr MPP09Reader::PROJECT2010_TASK_META_DATA_BIT_FLAGS;
//MppBitFlagArrayPtr MPP09Reader::PROJECT2013_TASK_META_DATA_BIT_FLAGS;
//MppBitFlagArrayPtr MPP09Reader::PROJECT2010_TASK_META_DATA2_BIT_FLAGS;
//MppBitFlagArrayPtr MPP09Reader::PROJECT2013_TASK_META_DATA2_BIT_FLAGS;
//MppBitFlagArrayPtr MPP09Reader::PROJECT2010_RESOURCE_META_DATA2_BIT_FLAGS;
//MppBitFlagArrayPtr MPP09Reader::PROJECT2013_RESOURCE_META_DATA2_BIT_FLAGS;
//MppBitFlagArrayPtr MPP09Reader::PROJECT2010_RESOURCE_META_DATA_BIT_FLAGS;
//MppBitFlagArrayPtr MPP09Reader::PROJECT2013_RESOURCE_META_DATA_BIT_FLAGS;

MPP09Reader::MPP09Reader()
{
	//INIT_STATIC_VAR_META_DATA_BIT_FLAGS();
}

//void MPP09Reader::INIT_STATIC_VAR_META_DATA_BIT_FLAGS()
//{
...
*/
void MPP09Reader::process(MPPReader* reader, ProjectFile* file, DirectoryEntryPtr root)
{
	try
	{
...
// 获取不同版本参数
const MPPVersionParams* MPP09Reader::getVersionParams()
{
	static MPPVersionParams params = { 0 };
	if (params.FileVersion == 0) {
		params.FileVersion = Project2003;
		params.PropsEntry = L"Props9";
		params.ProjectDirEntry = L"   19";
	}

	return &params;
}

VarMetaPtr MPP09Reader::newVarMetaObj(BwProj::toolkit::IDataReadStream* is)
{
	return VarMetaPtr(new VarMeta9(is));
}
/**
...
* @param root Root of the POI file system.
*/
void MPP09Reader::populateMemberData(MPPReader* reader, ProjectFile* file, DirectoryEntryPtr root)
{
	m_reader = reader;
	m_file = file;
	m_root = root;
	...
	//FieldMap fm = new FieldMap9(m_file);
...
*/
void MPP09Reader::clearMemberData()
{
	m_reader = NULL;
	//m_file = null;
	//m_root = null;
...
*/
void MPP09Reader::processCustomValueLists()
{
...
*/
void MPP09Reader::processProjectHeader()
{
...
//*/
//void MPP09Reader::processGraphicalIndicators()
//{
...
* subprojects.
*/
void MPP09Reader::processSubProjectData()
{
...
*/
void MPP09Reader::readSubProjects(ByteArrayPtr data, int uniqueIDOffset, int filePathOffset, int fileNameOffset, int subprojectIndex)
{
...
*/
SubProjectPtr MPP09Reader::readSubProject(ByteArrayPtr data, int uniqueIDOffset, int filePathOffset, int fileNameOffset, int subprojectIndex)
{
	try
	{
...
//*/
//void MPP09Reader::processViewPropertyData()
//{
...
//*/
//void MPP09Reader::processBaseFonts(ByteArrayPtr data)
//{
//    int offset = 0;
//
...
*/
void MPP09Reader::processTaskFieldNameAliases(ByteArrayPtr data)
{
	if (!data)
		return;
...
*/
void MPP09Reader::processResource2FieldNameAliases(ByteArrayPtr data)
{
	if (!data)
		return;
...
*/
std::shared_ptr<TreeMap<int, int>> MPP09Reader::createTaskMap(FieldMap* fieldMap, FixedMetaPtr taskFixedMeta, FixedDataPtr taskFixedData)
{
...
*/
std::shared_ptr<TreeMap<int, int>> MPP09Reader::createResource2Map(FieldMap* fieldMap, FixedMetaPtr rscFixedMeta, FixedDataPtr rscFixedData)
{
...
* to the calendar.
*/
void MPP09Reader::processCalendarData()
{
	...
	for (int loop = 0; loop < items; loop++)
	{
		...
		while (offset + 12 <= (int)fixedData->length())
		{
			...
			if (calendarID > 0 && baseCalendarID != 0 && calendarMap.containsKey(calendarID) == false)
			{
...
*/
void MPP09Reader::processCalendarHours(ByteArrayPtr data, ProjectCalendarPtr defaultCalendar, ProjectCalendarPtr cal, bool isBaseCalendar)
{
	...
	for (int index = 0; index < 7; index++)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					DatePtr start = MPPUtility::getTime(data.get(), offset + 8 + (periodIndex * 2));
					duration = MPPUtility::getDuration(data.get(), offset + 20 + (periodIndex * 4));
					hours->addRange(DateRangePtr(new DateRange(start, DatePtr (new Date(start->getTime() + duration)))));
...
* @param cal calendar instance
*/
void MPP09Reader::processCalendarExceptions(ByteArrayPtr data, ProjectCalendarPtr cal)
{
	int exceptionCount = MPPUtility::getShort(data.get(), 0);
	...
	{
		int offset = 4 + (60 * 7) + (index * 64);

		DatePtr fromDate = MPPUtility::getDate(data.get(), offset);
		DatePtr toDate = MPPUtility::getDate(data.get(), offset + 2);
		ProjectCalendarExceptionPtr exception = cal->addCalendarException(fromDate, toDate);

		int periodCount = MPPUtility::getShort(data.get(), offset + 6);
		if (periodCount <= 0)
			continue;

		for (int exceptionPeriodIndex = 0; exceptionPeriodIndex < periodCount; exceptionPeriodIndex++)
		{
			DatePtr start = MPPUtility::getTime(data.get(), offset + 12 + (exceptionPeriodIndex * 2));
			long duration = MPPUtility::getDuration(data.get(), offset + 24 + (exceptionPeriodIndex * 4));
...
*/
void MPP09Reader::updateBaseCalendarNames(
...
//获取一个整数
int CompIntValue2(const void *a, const void *b)
{
	return *(int*)a - *(int*)b;
}
...
*/
void MPP09Reader::processTaskData()
{
	FieldMapPtr fieldMap(new FieldMap9(m_file));
	fieldMap->createTaskFieldMap(m_projectProps);
//#ifdef _DEBUG
//	fieldMap->debugPrintFieldMap();
//#endif
	...
	// Process aliases      
	processTaskFieldNameAliases(props->getByteArray2(Props::TASK_FIELD_NAME_ALIASES));
	...
	IntArray2Ptr uniqueIdArray = taskMap->keySet()->toArray(); //taskVarMeta.getUniqueIdentifierArray();
															  // 排序
	std::qsort(uniqueIdArray->get(), uniqueIdArray->length(), sizeof(int), CompIntValue2);
	...
	for (size_t loop = 0; loop < uniqueIdArray->length(); loop++)
	{
		...
		if (uniqueID != 0 && !taskVarMeta->containsKey(uniqueID))
		{
			continue;
		}
		...
		task->disableEvents();
		ByteArrayPtr populateArr[] = { data };
		fieldMap->populateContainer(L"TaskField", task, uniqueID, populateArr, 1, taskVarData);
		...
		task->setID(id);
		task->setIgnoreResource2Calendar((((*metaData)[10] & 0x02) != 0));
		...
		task->setOutlineCode(1, m_outlineCodeVarData->getUnicodeString(
			NumberHelper::getInt(task->getCachedValue(TaskField::TF_OUTLINE_CODE1_INDEX)), OUTLINECODE_DATA));
		task->setOutlineCode(2, m_outlineCodeVarData->getUnicodeString(
			NumberHelper::getInt(task->getCachedValue(TaskField::TF_OUTLINE_CODE2_INDEX)), OUTLINECODE_DATA));
		task->setOutlineCode(3, m_outlineCodeVarData->getUnicodeString(
			NumberHelper::getInt(task->getCachedValue(TaskField::TF_OUTLINE_CODE3_INDEX)), OUTLINECODE_DATA));
		task->setOutlineCode(4, m_outlineCodeVarData->getUnicodeString(
			NumberHelper::getInt(task->getCachedValue(TaskField::TF_OUTLINE_CODE4_INDEX)), OUTLINECODE_DATA));
		task->setOutlineCode(5, m_outlineCodeVarData->getUnicodeString(
			NumberHelper::getInt(task->getCachedValue(TaskField::TF_OUTLINE_CODE5_INDEX)), OUTLINECODE_DATA));
		task->setOutlineCode(6, m_outlineCodeVarData->getUnicodeString(
			NumberHelper::getInt(task->getCachedValue(TaskField::TF_OUTLINE_CODE6_INDEX)), OUTLINECODE_DATA));
		task->setOutlineCode(7, m_outlineCodeVarData->getUnicodeString(
			NumberHelper::getInt(task->getCachedValue(TaskField::TF_OUTLINE_CODE7_INDEX)), OUTLINECODE_DATA));
		task->setOutlineCode(8, m_outlineCodeVarData->getUnicodeString(
			NumberHelper::getInt(task->getCachedValue(TaskField::TF_OUTLINE_CODE8_INDEX)), OUTLINECODE_DATA));
		task->setOutlineCode(9, m_outlineCodeVarData->getUnicodeString(
			NumberHelper::getInt(task->getCachedValue(TaskField::TF_OUTLINE_CODE9_INDEX)), OUTLINECODE_DATA));
		task->setOutlineCode(10, m_outlineCodeVarData->getUnicodeString(
			NumberHelper::getInt(task->getCachedValue(TaskField::TF_OUTLINE_CODE10_INDEX)), OUTLINECODE_DATA));
		...
		//	task->getTaskMode() == TaskMode::AUTO_SCHEDULED))
		//{
		//	task->setDuration(std::dynamic_pointer_cast<Duration>(task->getCachedValue(TaskField::TF_SCHEDULED_DURATION)));
		//}
		...
		//
		SubProjectPtr sp;
		if (m_taskSubProjects.Lookup(task->getUniqueID(), sp))
		{
...
*/
void MPP09Reader::postProcessTasks()
{
	List<Task*>* allTasks = m_file->getAllTasks();
	if (allTasks->GetCount() <= 1)
		return;

	Collections::sort_ptr_list(*allTasks);

	int taskID = -1;
	int lastTaskID = -1;

	for (int i = 0; i < (int) allTasks->GetCount(); i++)
	{
	...
	//std::shared_ptr<Array<__int64>> orderArray = m_taskOrder.keySet()->toArray();
...
*/
void MPP09Reader::processTaskEnterpriseColumns(int id, Task* task, Var2DataPtr taskVarData, ByteArrayPtr metaData2)
{
	if (!metaData2)
		return;
...
*/
void MPP09Reader::processResource2EnterpriseColumns(FieldMapPtr fieldMap, Resource2* resource, Var2DataPtr resourceVarData)
{
	IntegerPtr varDataKey = fieldMap->getVarDataKey(Resource2FieldClass::getInstance (RF_ENTERPRISE_DATA));
	//if (varDataKey != null)
	//{
	if (!varDataKey) return;
	ByteArrayPtr data = resourceVarData->getByteArray2(resource->getUniqueID(), *varDataKey);
	//}

	if (!data) return;

	PropsBlock props (data);
	//System.out.println(props);
	resource->setCreationDate(props.getTimestamp(Props::RESOURCE_CREATION_DATE));

	std::shared_ptr<Array2<int>> propKeySet = props.keySet()->toArray();
	for (int i = 0; i < (int) propKeySet->length(); i++)
	{
		int key = (*propKeySet)[i];
		int keyValue = key - MPPResource2Field::RESOURCE_FIELD_BASE;
		//System.out.println("Key=" + keyValue);

		FieldType* field = Resource2FieldClass::getInstance(keyValue);
		...
		case DT_CURRENCY:
		{
			value.reset (new Double(props.getDouble(key) / 100));
			break;
		}
		...
		case DT_DURATION:
		{
			ByteArrayPtr durationData = props.getByteArray2(key);
			double durationValueInHours = ((double)MPPUtility::getInt(durationData.get(), 0)) / 600;
			...
			{
				durationUnits = MPPUtility::getDurationTimeUnits(MPPUtility::getShort(durationData.get(), 4));
			}
			DurationPtr duration = Duration2::getInstance(durationValueInHours, TimeUnit::HOURS);
		...
		case DT_BOOLEAN:
		{
			if (field && field->getValue() == Resource2Field::RF_FLAG1)
			{
				field = NULL;
				int bits = props.getInt(key);
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG1, BooleanPtr(new Boolean((bits & 0x00002) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG2, BooleanPtr(new Boolean((bits & 0x00004) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG3, BooleanPtr(new Boolean((bits & 0x00008) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG4, BooleanPtr(new Boolean((bits & 0x00010) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG5, BooleanPtr(new Boolean((bits & 0x00020) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG6, BooleanPtr(new Boolean((bits & 0x00040) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG7, BooleanPtr(new Boolean((bits & 0x00080) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG8, BooleanPtr(new Boolean((bits & 0x00100) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG9, BooleanPtr(new Boolean((bits & 0x00200) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG10, BooleanPtr(new Boolean((bits & 0x00400) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG11, BooleanPtr(new Boolean((bits & 0x00800) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG12, BooleanPtr(new Boolean((bits & 0x01000) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG13, BooleanPtr(new Boolean((bits & 0x02000) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG14, BooleanPtr(new Boolean((bits & 0x04000) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG15, BooleanPtr(new Boolean((bits & 0x08000) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG16, BooleanPtr(new Boolean((bits & 0x10000) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG17, BooleanPtr(new Boolean((bits & 0x20000) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG18, BooleanPtr(new Boolean((bits & 0x40000) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG19, BooleanPtr(new Boolean((bits & 0x80000) != 0)));
				resource->set(Resource2Field::RF_ENTERPRISE_FLAG20, BooleanPtr(new Boolean((bits & 0x100000) != 0)));
			}
			break;
		}
		...
		case DT_NUMERIC:
		{
			value.reset (new Double (props.getDouble(key)));
			break;
		}

		case DT_STRING:
		{
			value.reset (new Variant (props.getUnicodeString(key)));
			break;
		}
...
*/
void MPP09Reader::processExternalTasks(List<Task*>* externalTasks)
{
	//
...
*/
void MPP09Reader::processHyperlinkData(Task* task, ByteArrayPtr data)
{
	if (!data)
		return;
...
* @param data hyperlink data block
*/
void MPP09Reader::processHyperlinkData(Resource2Ptr resource, ByteArrayPtr data)
{
	if (!data) return;
...
* @throws java.io.IOException
*/
void MPP09Reader::processConstraintData()
{
...
* @throws java.io.IOException
*/
void MPP09Reader::processResource2Data()
{
	FieldMapPtr fieldMap(new FieldMap9(m_file));
	fieldMap->createResource2FieldMap(m_projectProps);

	FieldMapPtr enterpriseCustomFieldMap(new FieldMap9(m_file));
	...
	// Process aliases      
	processResource2FieldNameAliases(m_projectProps->getByteArray2(Props::RESOURCE_FIELD_NAME_ALIASES));
	...
	for (size_t loop = 0; loop < uniqueid->length(); loop++)
	{
		...
		ByteArrayPtr arr2[] = { data };
		fieldMap->populateContainer(L"Resource2Field", resource.get(), id, arr2, 1, rscVarData);
		...
		// Process any enterprise columns
		//         
		processResource2EnterpriseColumns(fieldMap, resource.get(), rscVarData);
...
* @throws IOException
*/
void MPP09Reader::processAssignmentData()
{
	FieldMapPtr fieldMap(new FieldMap9(m_file));
	...
	FixedDataPtr assnFixedData(new FixedData(142, EncryptedDocumentInputStream::getInstance(m_file, assnDir, L"FixedData").get()));
...
*/
bool MPP09Reader::getDurationEstimated(int type)
{
...
* @return item value
*/
CString MPP09Reader::getCustomFieldOutlineCodeValue(Var2DataPtr varData, Var2DataPtr outlineCodeVarData, int id, int type)
{
	CString result;
...
* @return item value
*/
CString MPP09Reader::getCustomFieldOutlineCodeValue(Var2DataPtr varData, Var2DataPtr outlineCodeVarData, int id)
{
	CString result;
...
*/
void MPP09Reader::readBitFields(MppBitFlagArrayPtr flags, FieldContainer* container, ByteArrayPtr data)
{
