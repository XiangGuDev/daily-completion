...
namespace BwProj {
	namespace toolkit {
		...
		*/
		...
		class COMMONTOOLS_API CXmlAttribute;
		class COMMONTOOLS_API CXmlElement;
		class COMMONTOOLS_API CXmlDocument;
...
#include "XmlDocument.hpp"
		...
		//! CXmlElements
		class COMMONTOOLS_API CXmlElements : public CXmlContainer<CXmlElements, CXmlElement>
		{
		...
		//! CXmlAttributes
		class COMMONTOOLS_API CXmlAttributes : public CXmlContainer<CXmlAttributes, CXmlAttribute>
		{
			friend class CXmlElement;
		public:
...
#define XMD_FLAGS_SAVEFORMAT_ANSI				0x01
	...
	//! 文档处理
		...
		public:
			...
			* - TRUE 加载成功， FALSE 加载失败
			*/
			BOOL LoadStream(IRandomReadStream *pDataStream, DocFormatEnum format);
			...
			* - TRUE 保存成功， FALSE 保存失败
			*/
			BOOL SaveStream(IRandomWriteStream *pDataStream, DocFormatEnum format);
