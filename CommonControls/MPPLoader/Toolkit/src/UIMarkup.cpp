...
{
	if (m_pOwner == NULL || (size_t)m_iPos >= m_pOwner->m_nElements) return NULL;
...
#ifdef _UNICODE
	// ת����UNICODE
	LPSTR strXml = static_cast<LPSTR> (malloc(dwSize + 1));
	::ReadFile(hFile, strXml, dwSize, &dwRead, NULL);
	strXml[dwSize] = '\0';
	CA2W fileXml(strXml);
	free (strXml);
	::CloseHandle(hFile);
...
bool CMarkup::_ParseData(LPTSTR& pstrText, LPTSTR& pstrDest, TCHAR cEnd)
{
...
bool CMarkup::_Failed(LPCTSTR pstrError, LPCTSTR pstrLocation)
{
	...
	//TRACE(pstrLocation);
	_tcsncpy_s(m_szErrorMsg, 100, pstrError, (sizeof(m_szErrorMsg) / sizeof(m_szErrorMsg[0])) - 1);
	_tcsncpy_s(m_szErrorXML, 100, pstrLocation != NULL ? pstrLocation : _T(""), lengthof(m_szErrorXML) - 1);
