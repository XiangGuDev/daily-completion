#include "stdafx.h"
#include "..\include\AllocMemoryPool2.h"
...
namespace BwProj {
	namespace toolkit {
		...
		{
			BwProj::toolkit::CCriticalSectionSync mutex(_CriticalMutex);
		...
		LPVOID CAllocMemoryPool::AllocChunk(PCHUNKHEAD chunk)
		{
			...
			// 计算内存块所在段，并设置段值
			PCHUNKHEAD group = GetGroup2ChunkHead(chunk->pMemory, chunk->nGroup2Index);
			group->nGroup2NotAlloc--;

			chunk->pMemory->LastOpGroup2 = chunk->nGroup2Index;
			...
			const BOOL& CalcBestBlockSize)
		{
			...
			size_t Group2Count = 0;
			size_t bestChunkCount = CalculateBestMemoryChunkCount(nGrowBy, Group2Count);

			// 分配的组数不能大于 256
			ATLASSERT(Group2Count <= MAXBYTE);
			...
			memory->Group2Count = Group2Count;
			memory->Group2Length = Group2Count;
			memory->LastOpGroup2 = 0;
			memory->LastOpChunk = 0;

			// 初使化每个结点的内容
			for (size_t i = 0; i < memory->Group2Count; i++)
			{
				for (size_t j = 0; j < memory->Group2Length; j++)
				{
					PCHUNKHEAD chunk = GetMemoryChunkHead(memory, i, j);

					if (j == 0)
					{
						chunk->nGroup2NotAlloc = (UCHAR)memory->Group2Length;
					}

					chunk->pMemory = memory;
					chunk->nGroup2Index = (UCHAR)i;
		...
		// 取得第N组的组结点
		CAllocMemoryPool::PCHUNKHEAD CAllocMemoryPool::GetGroup2ChunkHead(
		...
		CAllocMemoryPool::PCHUNKHEAD CAllocMemoryPool::GetMemoryChunkHead(
			CAllocMemoryPool::PMEMORYHEAD memory, size_t Group2Index, size_t ChunkIndex)
		{
			// 计算内存块所在索引
			size_t index = Group2Index * memory->Group2Length + ChunkIndex;
			...
			const size_t& RequestedSize)
		{
			...
			// 先查找组（一定找得到）
			size_t Group2Index = bestMemory->LastOpGroup2;
			if (GetGroup2ChunkHead(bestMemory, Group2Index)->nGroup2NotAlloc == 0)
			{
				for (size_t i = 0; i < bestMemory->Group2Count; i++)
				{
					PCHUNKHEAD group = GetGroup2ChunkHead(bestMemory, i);
					if (group->nGroup2NotAlloc)
					{
						Group2Index = i;
						break;
					}
			...
			for (size_t i = bestMemory->LastOpChunk; i < bestMemory->Group2Length; i++)
			{
				chunk = GetMemoryChunkHead(bestMemory, Group2Index, i);
				if (!chunk->bAlloc)
					return TRUE;
			}
		...
		// 计算最佳的块数量
		size_t CAllocMemoryPool::CalculateBestMemoryChunkCount(const size_t& GrowBy, size_t& Group2Count)
		{
			Group2Count = (size_t)ceil(sqrtl((long)GrowBy));
			return (size_t)powl((long)Group2Count, 2);
		}
	}
}