...
// 控件类名
#define SPLITCTRL_CLASS L"BwSplitCtrl"

IMPLEMENT_DYNAMIC(CBwSplitCtrl, CWnd)

BEGIN_MESSAGE_MAP(CBwSplitCtrl, CWnd)
...
bool CBwSplitCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, bool bVertical, DWORD dwStyle)
{
	bool bSuccess = false;
	// 水平还是垂直滚动条;
...
void CBwSplitCtrl::SetSplitterRect(LPRECT lpRect /*= NULL*/, bool bUpdate /*= true*/)
{
	if (lpRect == NULL)
	{
...
void CBwSplitCtrl::GetSplitterRect(LPRECT lpRect) const
{
	ATLASSERT(lpRect != NULL);
	*lpRect = m_rcSplitter;
}

bool CBwSplitCtrl::SetSplitterPos(int xyPos /*= -1*/, bool bUpdate /*= true*/)
{
	if (xyPos == -1)   // -1 == middle
	{
		if (t_bVertical)
...
void CBwSplitCtrl::SetSplitterPosPct(int nPct, bool bUpdate /*= true*/)
{
...
int CBwSplitCtrl::GetSplitterPos() const
{
	return m_xySplitterPos;
}

bool CBwSplitCtrl::SetSinglePaneMode(int nPane /*= SPLIT_PANE_NONE*/)
{
...
int CBwSplitCtrl::GetSinglePaneMode() const
{
	return m_nSinglePane;
}

DWORD CBwSplitCtrl::GetSplitterExtendedStyle() const
{
	return m_dwExtendedStyle;
}

DWORD CBwSplitCtrl::SetSplitterExtendedStyle(DWORD dwExtendedStyle, DWORD dwMask /*= 0*/)
{
...
void CBwSplitCtrl::SetSplitterPanes(HWND hWndLeftTop, HWND hWndRightBottom, bool bUpdate)
{
...
bool CBwSplitCtrl::SetSplitterPane(int nPane, HWND hWnd, bool bUpdate /*= true*/)
{
...
HWND CBwSplitCtrl::GetSplitterPane(int nPane) const
{
	ATLASSERT(nPane == SPLIT_PANE_LEFT || nPane == SPLIT_PANE_RIGHT);
...
bool CBwSplitCtrl::SetActivePane(int nPane)
{
	ATLASSERT(nPane == SPLIT_PANE_LEFT || nPane == SPLIT_PANE_RIGHT);
...
int CBwSplitCtrl::GetActivePane() const
{
...
bool CBwSplitCtrl::ActivateNextPane(bool bNext /*= true*/)
{
...
bool CBwSplitCtrl::SetDefaultActivePane(int nPane)
{
	ATLASSERT(nPane == SPLIT_PANE_LEFT || nPane == SPLIT_PANE_RIGHT);
...
bool CBwSplitCtrl::SetDefaultActivePane(HWND hWnd)
{
	for (int nPane = 0; nPane < m_nPanesCount; nPane++)
	{
...
int CBwSplitCtrl::GetDefaultActivePane() const
{
	return m_nDefActivePane;
}

void CBwSplitCtrl::DrawSplitter(CDC &dc)
{
...
void CBwSplitCtrl::DrawSplitterBar(CDC &dc)
{
	RECT rect;
...
void CBwSplitCtrl::DrawSplitterPane(CDC &dc, int nPane)
{
	RECT rect;
...
void CBwSplitCtrl::SetWidth(int nWidth)
{
	if (nWidth < 1)
		return;
	m_cxySplitBar = nWidth;
}

LRESULT CBwSplitCtrl::OnCreate(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
...
LRESULT CBwSplitCtrl::OnPaint(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
...
LRESULT CBwSplitCtrl::OnSetCursor(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CBwSplitCtrl::OnMouseMove(WPARAM wParam, LPARAM lParam)
{
	if (_bFixed)
		return 0;
...
LRESULT CBwSplitCtrl::OnLButtonDown(WPARAM /*wParam*/, LPARAM lParam)
{
	if (_bFixed)
		return 1;
...
LRESULT CBwSplitCtrl::OnLButtonUp(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	::ReleaseCapture();
...
LRESULT CBwSplitCtrl::OnLButtonDoubleClick(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	this->SetSplitterPos();   // middle
	return 0;
}

LRESULT CBwSplitCtrl::OnCaptureChanged(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	if (!m_bFullDrag)
	{
		DrawGhostBar();
...
LRESULT CBwSplitCtrl::OnSetFocus(WPARAM /*wParam*/, LPARAM)
{
	if (m_nSinglePane == SPLIT_PANE_NONE)
	{
...
LRESULT CBwSplitCtrl::OnMouseActivate(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CBwSplitCtrl::OnSettingChange(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	GetSystemSettings(true);
	return 0;
}

LRESULT CBwSplitCtrl::OnEraseBackground(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	// handled, no background painting needed
	return 1;
}

LRESULT CBwSplitCtrl::OnSize(WPARAM wParam, LPARAM /*lParam*/)
{
...
void CBwSplitCtrl::UpdateSplitterLayout()
{
	if (m_nSinglePane == SPLIT_PANE_NONE && m_xySplitterPos == -1)
		return;
...
bool CBwSplitCtrl::GetSplitterBarRect(LPRECT lpRect) const
{
	ATLASSERT(lpRect != NULL);
	if (m_nSinglePane != SPLIT_PANE_NONE || m_xySplitterPos == -1)
		return false;
...
bool CBwSplitCtrl::GetSplitterPaneRect(int nPane, LPRECT lpRect) const
{
	ATLASSERT(nPane == SPLIT_PANE_LEFT || nPane == SPLIT_PANE_RIGHT);
	ATLASSERT(lpRect != NULL);
	bool bRet = true;
...
bool CBwSplitCtrl::IsOverSplitterRect(int x, int y) const
{
	// -1 == don't check
...
bool CBwSplitCtrl::IsOverSplitterBar(int x, int y) const
{
	if (m_nSinglePane != SPLIT_PANE_NONE)
		return false;
...
void CBwSplitCtrl::DrawGhostBar()
{
	RECT rect = { 0, 0, 0, 0 };
	if (GetSplitterBarRect(&rect))
	{
...
void CBwSplitCtrl::GetSystemSettings(bool bUpdate)
{
...
bool CBwSplitCtrl::IsProportional() const
{
	return ((m_dwExtendedStyle & SPLIT_PROPORTIONAL) != 0);
}

void CBwSplitCtrl::StoreProportionalPos()
{
...
void CBwSplitCtrl::UpdateProportionalPos()
{
	int cxyTotal = t_bVertical ? (m_rcSplitter.right - m_rcSplitter.left - m_cxySplitBar - m_cxyBarEdge) : (m_rcSplitter.bottom - m_rcSplitter.top - m_cxySplitBar - m_cxyBarEdge);
	if (cxyTotal > 0)
	{
...
bool CBwSplitCtrl::IsRightAligned() const
{
	return ((m_dwExtendedStyle & SPLIT_RIGHTALIGNED) != 0);
}

void CBwSplitCtrl::StoreRightAlignPos()
{
	int cxyTotal = t_bVertical ? (m_rcSplitter.right - m_rcSplitter.left - m_cxySplitBar - m_cxyBarEdge) : (m_rcSplitter.bottom - m_rcSplitter.top - m_cxySplitBar - m_cxyBarEdge);
	if (cxyTotal > 0)
...
void CBwSplitCtrl::UpdateRightAlignPos()
{
	int cxyTotal = t_bVertical ? (m_rcSplitter.right - m_rcSplitter.left - m_cxySplitBar - m_cxyBarEdge) : (m_rcSplitter.bottom - m_rcSplitter.top - m_cxySplitBar - m_cxyBarEdge);
	if (cxyTotal > 0)
	{
...
bool CBwSplitCtrl::IsInteractive() const
{
	return ((m_dwExtendedStyle & SPLIT_NONINTERACTIVE) == 0);
}

bool CBwSplitCtrl::RegisterTreeList()
{
...
void CBwSplitCtrl::SetFixed(bool bFixed)
{
	_bFixed = bFixed;
}

LRESULT CBwSplitCtrl::OnMouseWheelEx(WPARAM wParam, LPARAM lParam)
{
