	...
	else
	{
		...
		if (leatValues.empty() && !((GetStyle() & CBS_DROPDOWNLIST) == CBS_DROPDOWNLIST))
		{
			...
			if (!strText.IsEmpty())
			{
				...
				CStringToolkitEx::SplitString((LPCTSTR)strText, strSplitStrings, (LPCTSTR)GetSplitChar());
				
				GetTreeLeafValueString(_dropTree, leatValues, true);
				CString strLeafValue = CStringToolkitEx::BuildSplitString(leatValues, (LPCTSTR)GetSplitChar());
				...
				strText = CStringToolkitEx::BuildSplitString(strSplitStrings, (LPCTSTR)GetSplitChar());
			}

		}
		else
		{
			strText = CStringToolkitEx::BuildSplitString(leatValues, (LPCTSTR)GetSplitChar());
		}
	}
...
CString CTreeComboBox::GetCurrentSelData()
{
	...
	else
	{
		...
		strText = CStringToolkitEx::BuildSplitString(leftDatas, (LPCTSTR)GetSplitChar());
	}
...
HTREEITEM CTreeComboBox::FindItemByText(HTREEITEM hRoot, CString strItem, CString strData)
{
	while (NULL != hRoot)
	{
		...
		// 比较关键字是否相同
		if (strData == strFindData && (!_bSelectByText || strFindItem == strItem))
			return hRoot;
...
void CTreeComboBox::DisplayTree()
{
	...
	else
	{
		...
		strSelData = GetCurrentSelData();
		CStringToolkitEx::SplitString((LPCTSTR)strSelText, strSelects, (LPCTSTR)GetSplitChar());
		CStringToolkitEx::SplitString((LPCTSTR)strSelData, strDatas, (LPCTSTR)GetSplitChar());
...
bool CTreeComboBox::LoadXmlData(const CString& xmlFile, bool bNoLimitOrSelAll)
{
	...
	if (CFileToolkit::FileExist((LPCTSTR)xmlFile))
	{
		// 加载文件失败
