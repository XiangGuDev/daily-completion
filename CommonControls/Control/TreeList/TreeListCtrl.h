...
*  @brief
*/
typedef struct COMMONCONTROLS_API tagCOLUMNDATA
{
	friend class CTreeListCtrl;
	friend class CTimeLineTreeListCtrl;
...
{
	friend class CTreeListCtrl;
	friend class CTimeFrameListCtrl;
	friend class CTimeLineTreeListCtrl;
public:
	tagITEMDATA()
	{
...
*  @brief
*/
class COMMONCONTROLS_API CTreeListCtrl : public CWnd
{
public:
	CTreeListCtrl();
...
public:
	...
	*			调用此方法用于更新界面显示与滚动条
	*/
	void UpdateList(bool bDataChange = false);
	...
	*  @brief    设置控件图标
	*
	*  @param    CImageList imageList 控件图标集
	*/
...
private:
	...
	*/
	virtual void UpdateScrollBar(int scrollMode = SB_BOTH);
...
private:
	...
	*/
	virtual void CreateEditorControl(int nOption, 
