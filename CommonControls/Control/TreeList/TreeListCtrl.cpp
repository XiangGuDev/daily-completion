...
void CTreeListCtrl::UpdateList(bool bDataChange)
{
	// 如果正在编辑则取消
	CancelEdit();
...
void CTreeListCtrl::SetImageList(CImageList &imageList)
{
	_imageList.DeleteImageList();

	_imageList.Attach(imageList.GetSafeHandle());
}
...
void CTreeListCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	if (_hHoverRow != NULL)
	{
		...
		// 发送双击事件;
		NotifyParent(LCN_DBLCLICK, nCol, nRow, &hColumnData->editConfig);
...
void CTreeListCtrl::DrawColumn(CMemoryDC& dcMemory, const CRect& rcColumn, HCOLUMNDATA hColumnData)
{
	...
	else if (_imageList.m_hImageList && (hColumnData->image != -1))
	{
		bDrawIcon = true;
		_imageList.DrawEx(&dcMemory, hColumnData->image, rcIcon.TopLeft(), _iconSize,
	...
	if (NULL != hColumnData && !rcText.IsRectEmpty())
	{
		...
		if (_bColumnWordWarp)
		{
			UINT nFormat = hColumnData->headerformat | DT_EDITCONTROL | DT_WORDBREAK | DT_NOPREFIX;
		...
		if (!bHasDrawText)
		{
			dcMemory.DrawText(hColumnData->title, rcText,
				hColumnData->headerformat | DT_SINGLELINE | DT_VCENTER | DT_WORD_ELLIPSIS | DT_NOPREFIX);
		}
	}
	...
	const CRect& rcRow, ColumnDataVector& leafColumnVector, bool bFirstRow)
{
	...
	if (_hasGrid)
	{
		...
		for (size_t i = 0; i < nCount; ++i)
		{
			...
			if (i == 0)
			{
				dcMemory.MoveTo(hColumnData->colRect.left, rcRow.top);
				dcMemory.LineTo(hColumnData->colRect.left, rcRow.bottom);
			}

			dcMemory.MoveTo(hColumnData->colRect.right - 1, rcRow.top);
			dcMemory.LineTo(hColumnData->colRect.right - 1, rcRow.bottom);
		}
	...
	HITEMDATA hItemData, HCELLINFO hCellInfo, HEDITCONFIG hEditConfig, HCOLUMNDATA hColumnData, const CString& strCellText)
{
	...
	*/
	...
	if (bTreeNode)
	{
		int dotLineSpace = 12; // 树结构线间距
		int dashLineSpace = 15; // 树结构破折线长

		// 绘制树结构线
		if (_hasLine)
		{
			COLORREF dotColor = GetSysColor(COLOR_GRAYTEXT);
	...
	else if (_imageList.m_hImageList)
	{
		if (hEditConfig->image != -1)
		{
			bDrawIcon = true;
			_imageList.DrawEx(&dcMemory, hEditConfig->image, rcIcon.TopLeft(), _iconSize,
	...
	const CString& strDefaultData)
{
	...
	else if (nOption & eDropListCobEdit || nOption & eDropDownCobEdit)
	{
		...
		CString strXmlFile = CPathConfig::GetConfigPath();
	...
	else if (nOption & eTreeComboEdit || (nOption & eTreeComboManualEdit))
	{
		...
		else if (CFileToolkit::FileExist((LPCTSTR)strXmlFile))
		{
	...
	else if (nOption & eTreeCheckComboEdit || (nOption & eTreeCheckComboManualEdit))
	{
		...
		// 加载下拉数据
		CString strXmlFile = CPathConfig::GetConfigPath() + editConfig.comboDataFile;
...
void CTreeListCtrl::SetHasGrid(bool bShow /*= true*/)
{
	_hasGrid = bShow;
}
