...
namespace CommBase
{
	...
	*  @brief
	*/
	class COMMBASE_API CStringTool
	{
	public:
		...
		* @return 返回转换成的字符串
		*/
		static CString DoubleToStr(double value);
		...
		* @return 返回转换成的浮点型
		*/
		static double StrToDouble(const CString& str);
		...
		*/
		static int CompareVersion(CString version1, CString version2, int nIndex = 0);
		...
		*/
		static void SplitString(const CString& strNeedSplit,
			StringArray& strSplitStrings, CString strSep = _T("#"));

		/**
		*  @brief    将拆分的字符串合成
		*
		*  @param    const CAtlArray<CString>& strSplitStrings 拆分的字符串
		*  @param    CString strSep 分隔符
		*  @return   CString 返回合并后的字符串
		*/
		static CString BuildSplitString(const StringArray& strSplitStrings,
			CString strSep = _T("#"));
		...
		*
		*  @param    int num	数字
		*  @return   CString	生成的中文数字
		*/
		static CString GenerateChineseNumIn100(int num);

		/**
		*  @brief    判读字符窗是否为合法数字
		*
		*  @param    CString	输入检测的字符串
		*  @return   bool	复合要求返回true
		*/
		static bool IsDigit(const CString& strText);

		/**
		*  @brief    将ASCII文本转换为UNICODE文本
		*
		*  @param    CStringA 输入的ASCII文本
		*  @param    int 编码格式（如果需要转UTF8格式请传入CP_UTF8)
		*  @return   CString　输出的UNICODE文本
		*/
		static CString MCharToWChar(const CStringA& s, int nCodePage = CP_ACP);

		/**
		*  @brief    将UNICODE文本转换为ASCII文本
		*
		*  @param    CString 输入的UNICODE文本
		*  @param    int 编码格式（如果需要转UTF8格式请传入CP_UTF8)
		*  @return   CString　输出的ASCII文本
		*/
		static CStringA WCharToMChar(const CString& s, int nCodePage = CP_ACP);

		/**
		*  @brief    编号递增
		*
		*  @param    const TCHAR *lpszCurCode
		*  @return   CString
		*/
		static CString GetNextCode(const TCHAR *lpszCurCode);
		...
		*  @return   CString
		*/
		static CString GetPrevCode(const TCHAR *lpszCurCode);
		...
		*/
		...
		/**
		* 汉字字符串转换成拼音
		* @param [in] strText	字符串
		* @return	返回转换后的拼音字符串
		*/
		static CString GetLetter(CString strText)
		{
			...
			for (int i = 0; i < len; i++)
			{
				...
				{
					strValue = strValue + pStr[i];
					continue;
				}

				ucHigh = (unsigned char)pStr[i];
				ucLow = (unsigned char)pStr[i + 1];
				...
				CString strRes = FindLetter(nCode);
