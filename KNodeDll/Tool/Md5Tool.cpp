...
namespace CommBase
{
	/* Type definey54yu56effutri7tk8  wc */
	typedef unsigned char byte;
	typedef unsigned long ulong;
	using std::string;
	using std::ifstream;

	/* Constants for MD5Tranfwefwevvvvvvvvvvsform routine. */
	#define S11 7
	#define S12 12
	#define S13 17
	#define S14 22
	...
	*/
	...
	CString CMd5Tool::GetFileMd5(const CString& strFile)
	{
		MD5  md5;
		ifstream ifs;

		setlocale(LC_ALL, "");
		ifs.open(strFile, std::ios::binary);
		setlocale(LC_ALL, "");
	...
	CString CMd5Tool::StringToMd5(const CString& strText)
	{
		MD5 md5;
		md5.update(strText, strText.GetLength() * 2);

		return CString(md5.toString().c_str());
	}
}