...
#define PUGI__CHECK_ERROR(err, m)	{ if (*s == 0) PUGI__THROW_ERROR(err, m); }
	...
	BOOL CXmlDocument::LoadFile(LPCTSTR XmlFile, DocFormatEnum format)
	{
		// 打开文件
		CAtlFile file;
		if (file.Create(XmlFile, GENERIC_READ, 0, OPEN_EXISTING) != S_OK)
		{
			_LastError.Format(L"LastError:%d", GetLastError());
			return FALSE;
		}
	...
	BOOL CXmlDocument::SaveFile(LPCTSTR XmlFile, DocFormatEnum format)
	{
		// 创建文件，允许文件已存在
		CAtlFile file;
		if (file.Create(XmlFile, FILE_WRITE_DATA, 0, OPEN_ALWAYS) != S_OK)
			return FALSE;
