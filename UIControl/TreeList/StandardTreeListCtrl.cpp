...
namespace ControlUI
{
		...
		const HEDITCONFIG hEditConfig, const CString& strNewValue, CTreeListConfig* pConfig)
	{
		...
		if ((eVeNoEmpty & hEditConfig->option) && strNewValue.IsEmpty())
			return false;

		if ((eVeNoZero & hEditConfig->option) && !CStringTool::StrToDouble(strNewValue))
			return false;
	...
	void CStandardTreeListCtrl::SetStandardDataSource(CStandardDataSource *pStandardDataSource)
	{
		_pRootDataSource = pStandardDataSource;
		SetDataSource(_pRootDataSource, false);
	}

	ControlUI::CStandardDataSource* CStandardTreeListCtrl::GetStandardDataSource()
	{
		return _pRootDataSource;
	}
	...
	ControlUI::EDITCONFIG CStandardTreeListCtrl::GetCellConfig(size_t nRow, size_t nColumn)
	{
		// 获取数据源并设置单元格样式
		CStandardDataSource* pDataSource = GetDataSource(nRow, nColumn);

		return pDataSource->GetCellCfg(nColumn);
	}
