...
namespace ControlUI
{
	...
	END_DYN_ENUM(EFormatType);

	BEGIN_DYN_ENUM(EValidEditControlFlag)
	{
	...
	END_DYN_ENUM(EValidEditControlFlag);

	BEGIN_DYN_ENUM(ETreeComboEditControlFlag)
	{
	...
	END_DYN_ENUM(ETreeComboEditControlFlag);


	BEGIN_DYN_ENUM(EDateTimeEditFlag)
	{
	...
	END_DYN_ENUM(EDateTimeEditFlag);

	BEGIN_DYN_ENUM(EProgressControlFlag)
	{
		{ ePrgShowText, _T("显示进度文字") },
		{ NULL, NULL }
	}
	END_DYN_ENUM(EProgressControlFlag);
	...
	void CTreeListConfig::LoadTypeFilter(CXmlElement* pTypeFilter)
	{
		...
		// 读取过滤节点类型
		int nWord = CStringTool::CountWord((LPCTSTR)strTypeList, L',');
		for (int i = 1; i <= nWord; ++i)
		{
			int nType = _wtoi(CStringTool::ReadWord((LPCTSTR)strTypeList, i, L','));
	...
	{
		CString strText = pCellCfg->GetAttrValue(L"Image");
		if (!strText.IsEmpty())
		...
		//只显示图片居中
		strText = pCellCfg->GetAttrValue(L"OnlyImage");
		if (!strText.IsEmpty())
		{
			hEditConfig->bImageCenter = true;
			hEditConfig->image = _ttoi(strText);
		}
		else if (NULL != hColEditConfig)
		{
			hEditConfig->bImageCenter = false;
			hEditConfig->image = hColEditConfig->image;
		}
		...
		strText = pCellCfg->GetAttrValue(L"IsShowTips");
		if (!strText.IsEmpty())
			hEditConfig->isShowTips = (strText == L"是");
		else if (NULL != hColEditConfig)
			hEditConfig->isShowTips = hColEditConfig->isShowTips;
