...
#define MAX_TEXT_CHAR 12
	...
	void CAngleEdit::SetMinAngle(double dAngle)
	{
		_dMinAngle = dAngle;
	}

	void CAngleEdit::SetMaxAngle(double dAngle)
	{
		_dMaxAngle = dAngle;
	}
	...
	void CAngleEdit::ArcDegree2AngleDegree(double dArcDegree, int &nAngle, int &nMin, int &nSec)
	{
		double dAngle = 180.0f * dArcDegree / M_PI;
		nAngle = (int)(dAngle);
		double dMin = (dAngle - (double)nAngle)*60.0f;
		nMin = (int)(dMin);
		double dSec = dMin - (double)nMin;

		// 四舍五入
		double dTemp = dSec*60.0f;
		dTemp = dArcDegree > 0 ? (dTemp + 0.5f) : (dTemp - 0.5f);
		nSec = (int)dTemp;
	...
	double CAngleEdit::AngleDegreeString2ArcDegree(const CString &strDegreeVal, const CString &strMinuteVal, const CString &strSecondVal)
	{
		long nAngle = _ttol(strDegreeVal);
		long nMin = _ttol(strMinuteVal);
		long nSec = _ttol(strSecondVal);
	...
	BOOL CAngleEdit::ValidateInput(TCHAR inChar)
	{
		...
		if (strText.GetAt(0) == '-')		//判断是否为负
		{
			bNegative = true;
			strDegree = strText.Mid(1, degreePos);
		}
		else
		{
			strDegree = strText.Mid(0, degreePos);
		}
		...
		double dData = AngleDegreeString2ArcDegree(strDegree, strMinute, strSecond);
		dData = bNegative ? -dData : dData;
		if (dData > _dMaxAngle || dData < _dMinAngle)
			return TRUE; // 输入不合法
		...
		if (ptCharPos <= degreeCharPos)
		{
			...
			// 自动切换到下一输入框
			if (degreeVal > 99 || degreeVal < -99 || degreeVal > _dMaxAngle / 10 || degreeVal < _dMinAngle / 10)
	...
	void CAngleEdit::SetArcValue(double dVal)
	{
		...
		ArcDegree2AngleDegree(dVal, nAngle, nMin, nSec);
	...
	double CAngleEdit::GetAngleValue()
	{
		...
		return AngleDegreeString2ArcDegree(degreeVal, minuteVal, secondVal);
	}
