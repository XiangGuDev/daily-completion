...
// 取消编辑事件
#define LCN_CANCELEDIT				(WM_USER + 1320)
...
*/
	...
	*/
	...
	{
		eVeDecimal = 1,			///< 允许输入小数		1
		eVeNegative = 1 << 1,	///< 允许输入负数		2
		eVeDotNum = 1 << 2,		///< 限制输入小数位数	4
		eVeNoZero = 1 << 3,		///< 禁止输入0	！CValidEdit未实现！,在模态对话框中使用CValidEditEx可实现		8
		eVeNoEmpty = 1 << 4,	///< 不允许为空	！CValidEdit未实现！,在模态对话框中使用CValidEditEx可实现		16
		eVePreZero = 1 << 5,	///< 允许前置零			32	
	};
